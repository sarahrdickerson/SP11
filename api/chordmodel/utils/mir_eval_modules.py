...
def audio_file_to_features(audio_file, config):
    ...
    currunt_sec_hz = 0
    feature = np.empty((config.feature['n_bins'], 0))  # Correctly shaped empty array to match `tmp`
    while len(original_wav) > currunt_sec_hz + config.mp3['song_hz'] * config.mp3['inst_len']:
        ...
        print(f"Shape of feature before concat: {feature.shape}")  # Debug print
        print(f"Shape of tmp: {tmp.shape}")  # Debug print
